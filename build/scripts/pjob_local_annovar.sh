...
do          
	...
	then
		...
		time $RSYNC $RSYNCFLAGS $rsyncflags --include="*/" --include="*.vcf" --exclude="*" $from/$folder $to
		...
		mkdir $finalto && time $RSYNC $RSYNCFLAGS $rsyncflags --include="*/" --include="*.vcf" --exclude="*" $from/$folder $finalto
		#copy_from_sync_jobid=$(echo $copy_from_sync_jobid | cut -d'.' -f 1)
	...
	then
		...
		annovar.sh $folder $finalto $NCPUS
