...
mkdir -p $outDir && echo $outDir 

fastQCDir=${outDir}"/fastQC"
mkdir -p $fastQCDir && echo "FastQCDir is $fastQCDir"

fastQC_trim_Dir=${outDir}"/fastQC_trim"
mkdir -p $fastQC_trim_Dir && echo "FastQCDir_trim is $fastQC_trim_Dir" 

trim_Dir=${outDir}"/fastq_trim"
mkdir -p ${trim_Dir} && echo "TrimmedDir is $trim_Dir" 
...
then
  ...
  ILLUMINACLIP:/opt/trimmomatic/adapters/TruSeq3-PE-2.fa:2:30:10 SLIDINGWINDOW:4:20 MINLEN:40"
...
if  [ ! -f ${outDir}/${1}_hc.bam ]; then
    ...
    -ERC GVCF --max_alternate_alleles 3 --read_filter OverclippedRead --dbsnp $db_snp \
